...
if doraw
    ...
    if ismember(datanum,[3 5])
        ...
        calcium = brick.div(calcium,prctile(calcium,30)) - 1;
    end
...
if dotrain
    if datanum<=5
        ...
        spiketimes = brick.map(num2cell(spiketimes,1),@(x)x(~isnan(x)));
    else
        % need to be estimated
        dtspikes = .01;
        spiketimes = brick.timevector(spikecount,dtspikes);
    end
else
...
if dataflag==0
    ...
    spiketimes = brick.map(spiketimes,@(x)x(x<=10));
...
elseif dataflag>=11 && dataflag<=20
    ...
    if datanum==2
        spiketimes(1:20) = brick.map(spiketimes(1:20),@(x)x(x>=80 & x<=200));
        spiketimes(21:end) = brick.map(spiketimes(21:end),@(x)x(x<=120));
    else
        spiketimes = brick.map(spiketimes,@(x)x(x<=120));
    end
elseif dataflag==92
    %%
    ...
    if dotrain
        ...
        cc(1:2000,8) = cc(1:2000,8) - brick.column(linspace(5,0,2000));
        cc(849:985,8) = 0.45;
        idx=4901:6465; cc(idx,13) = cc(idx,13) - brick.column(linspace(5,0,length(idx)));
        cc(idx(1):5023,13) = .45;
        idx = 1:1051; cc(idx,14) = cc(idx,14) - brick.column(linspace(4,0,length(idx)));
        idx=440:580; cc(1:idx(1),14) = cc(1:idx(1),14)+5; cc(idx,14) = .8;
    else
        idx = 1:1757; cc(idx,6) = cc(idx,6) - brick.column(linspace(4,0,length(idx)));
        ...
        idx = 5620:7100;
        cc(idx,7) = cc(idx,7) - brick.column(linspace(3,0,length(idx)));
        cc(idx(1):5750,7) = 0;
    end
